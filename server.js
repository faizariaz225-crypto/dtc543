...
function loadEmailCfg() { try { return JSON.parse(fs.readFileSync(EMAIL_CONFIG, 'utf8')); } catch { return {}; } }
function saveEmailCfg(c){ fs.writeFileSync(EMAIL_CONFIG, JSON.stringify(c, null, 2)); }
function loadEmailLog() { try { return JSON.parse(fs.readFileSync(EMAIL_LOG,    'utf8')); } catch { return []; } }
...
setInterval(() => checkSubscriptionEmails().catch(e => console.error('[email-check error]', e.message)), 60*60*1000);
setTimeout(  () => checkSubscriptionEmails().catch(e => console.error('[email-check error]', e.message)), 30000);
...
app.post('/api/submit', (req, res) => {
  ...
  const isChatGPT   = (t.packageType||'').toLowerCase().includes('chatgpt');
  const errors = {};
  if (!orgId || orgId.trim().length === 0) {
    errors.orgId = 'This field is required.';
  } else if (!isChatGPT && !UUID_REGEX.test(orgId.trim())) {
    errors.orgId = 'Invalid Organization ID format.';
  }
